...
# Elaine --version
send_LIT101 = ('HR', 0, 'LIT101')
get_LIT101 = ('LIT101', 1)

send_MV101 = ('HR', 0, 'MV101')
set_MV101 = ('MV101', 1)

send_P101 = ('HR', 0, 'P101')
set_P101 = ('P101', 1)

send_FIT201_1 = ('HR', 0, 'FIT201')
rec_FIT201_2 = ('HR', 0, 'FIT201')

send_LIT301_1 = ('HR', 0, 'LIT301')
rec_LIT301_3 = ('HR', 0, 'LIT301') 
...
class SwatPLC1(PLC):
    ...
    def main_loop(self):
        ...
        while(count <= PLC_SAMPLES):
            ...
            # lit101 [meters]
            lit101 = int(float(self.get(get_LIT101)))
            print(('DEBUG plc1 lit101: %.5f' % lit101))
            self.send(send_LIT101, lit101, (PLC1_ADDR+ ':502'))
            ...
            if lit101 >= LIT_101_M['H']:
                ...
                self.set(set_MV101, 0)
                self.send(send_MV101, 0, (PLC1_ADDR+ ':502'))
                ...
                self.set(set_P101, 0)
                self.send(send_P101, 0, (PLC1_ADDR+ ':502'))

            elif lit101 <= LIT_101_M['L']:
                # OPEN mv101
                print ("INFO PLC1 - lit101 under L -> open mv101.")
                self.set(set_MV101, 1)
                self.send(send_MV101, 1, (PLC1_ADDR+ ':502'))

            # TODO: use it when implement raw water tank
            # read from PLC2 (constant value)
            fit201 = (self.receive(rec_FIT201_2, (PLC2_ADDR+ ':502')))
            if fit201 != None:
                print(("fit201", fit201))
                fit201 = int(float(fit201))
                print(("DEBUG PLC1 - receive fit201: %f" % fit201))
                self.send(send_FIT201_1, fit201, (PLC1_ADDR+ ':502'))

            # # read from PLC3
            lit301 = (self.receive(rec_LIT301_3, (PLC3_ADDR+ ':502')))
            if lit301 != None:
                print(("lit301", lit301))
                lit301 = int(float(lit301))
                print(("DEBUG PLC1 - receive lit301: %f" % lit301))
                self.send(send_LIT301_1, lit301, (PLC1_ADDR+ ':502'))
                ...
                if fit201 <= FIT_201_THRESH or lit301 >= LIT_301_M['H']:
                    ...
                    # OPEN p101
                    self.set(set_P101, 1)
                    self.send(send_P101, 1, (PLC1_ADDR+ ':502'))
